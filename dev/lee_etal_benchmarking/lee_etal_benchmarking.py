...
df_fres_const = pd.read_csv(
    filepath_or_buffer="data/fres_0-08.csv", header=0, index_col=None
)
df_fres_dist_time = pd.read_csv(
    filepath_or_buffer="data/fres_100nmi+45min.csv", header=0, index_col=None
)
df_fres_time = pd.read_csv(
    filepath_or_buffer="data/fres_45min.csv", header=0, index_col=None
)
...
for range in df_fres_const["range"]:
    ...
    results.append({"range": range, "payload": dict_results["mass_payload"]})
...
plt.savefig(fname=figure_name, format="pdf", bbox_inches="tight", transparent=False)
