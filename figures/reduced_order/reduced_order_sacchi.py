...
data = np.array([
    [0,0], [54, 136], [145, 10724], [254, 19140], [327, 23484],
    [400, 26200], [550, 26200], [650, 27000], [850, 27000],
    [950, 27800], [1100, 27800], [1213, 22127], [1345, 7738],
    [1444, 0], [1516, 0]
])
x_prof = data[:, 0]
y_prof = data[:, 1]
...
# 1. Scatter Data
ax1.scatter(
    weights_sim, slopes_sim, 
    s=50, c='#4C72B0', label='EMEP Data',
...
for vx in vline_x:
    ...
    ax1.vlines(x=vx, ymin=0, ymax=vy, colors='black', linestyles='--', linewidth=1, zorder=0)
    
    # Horizontal line (Red, Dashed)
    ax1.hlines(y=vy, xmin=0, xmax=vx, colors='red', linestyles='--', linewidth=1, zorder=0)
...
con_top = ConnectionPatch(
    xyA=(last_x, last_y), coordsA=ax1.transData,
    xyB=(bbox.x0, bbox.y1), coordsB='figure fraction',
...
con_bottom = ConnectionPatch(
    xyA=(last_x, last_y), coordsA=ax1.transData,
    xyB=(bbox.x0, bbox.y0), coordsB='figure fraction',
    color="black", linestyle=":", linewidth=1
)
